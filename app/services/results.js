angular.module('app.services.results', []).factory("Results", function(Tools, Locations) {
	function getResults(tools, locationID) {
		...
		for (count = 0; count < tools.length; count++) {
			if (tools[count].id == 1 && id_1 == false) {
				msg += "Solar panels are renewable, abundant, sustainable, environmentally friendly,
				...
				msg += "Hydroelectric energy is renewable, clean (no greenhouse gases produced), is
				...
				msg += "Wind energy is clean, does not use water in its energy generation, an inexhaustible
	...
	function getCost(tools, locationID) {
		...
		var location = Locations.getLocationByID(locationID);
		// TODO: some algorithm to generate cost per year according to given tools and location
		var count;
		var total = 0;
		for (count = 0; count < tools.length; count++) {
			if (tools[count].id == 1) {
